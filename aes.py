...
class Aes:
  ...
  def mix_column(col, inv=False):
    ret, m = [], mcs if not inv else imcs
    for i in range(0,16,4):
      val = 0
      for j in range(len(col)):
        val ^= Aes.gmul(m[i+j], col[j])
      ret.append(val)
    return ret

  def sub_bytes(self, state, inv=False):
    box = sbox if not inv else isbox
    self.foreach_in_state(state, lambda r, c: Aes.sub_byte(box, state[r][c]))

  def mix_columns(self, state, inv=False):
    state[:] = [Aes.mix_column(state[r], inv) for r in range(self.Nk)]

  def add_round_key(self, state, wbs):
    self.foreach_in_state(state, lambda r, c: state[r][c] ^ wbs[r * self.Nk + c])

  def foreach_in_state(self, state, fn):
    state[:] = [[fn(r, c) for c in range(self.Nb)] for r in range(self.Nk)]

  def shift_rows(self, state, inv=False):
    copy = [e[:] for e in state]
    self.foreach_in_state(state, lambda r, c: copy[(r+c if not inv else r-c) % self.Nk][c])
  ...
  def key_expansion(self, key):
    ...
    for i in range(len(ksb)):
      if i < Nk:
        ...
        temp = ksb[i - 1]
        if not i % Nk:
          temp = Aes.sub_word(sbox, Aes.rotate_R32(temp, 24)) ^ (rcon[i // Nk] << 24)
        elif Nk > 6 and i % Nk == 4:
          temp = Aes.sub_word(sbox, temp)
        ksb[i] = ksb[i - Nk] ^ temp
    return [byte for ui32 in ksb for byte in Aes.uint32_bytes(ui32)] # -> byte[n * 4]

  def cipher(self, state, ks):
    Nb, Nr, Nk = self.Nb, self.Nr, self.Nk
    self.add_round_key(state, ks[0:Nb*4])
    for r in range(1, Nr+1):
      self.sub_bytes(state)
      self.shift_rows(state)
      if r < Nr: self.mix_columns(state)
      self.add_round_key(state, ks[r*Nb*4:(r+1)*Nb*4])

  def inv_cipher(self, state, ks):
    Nb, Nr, Nk = self.Nb, self.Nr, self.Nk
    self.add_round_key(state, ks[Nr*Nb*4:(Nr+1)*Nb*4])
    for r in range(Nr-1, -1, -1):
      self.shift_rows(state, True)
      self.sub_bytes(state, True)
      self.add_round_key(state, ks[r*Nb*4:(r+1)*Nb*4])
      if r: self.mix_columns(state, True)
