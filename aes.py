...
class Aes:
  ...
  def mix_columns(state, inv=False):
    state[:] = [Aes.mix_column(state[r], inv) for r in range(BS)]

  def add_round_key(state, wbs):
    Aes.foreach_in_state(state, lambda r, c: state[r][c] ^ wbs[r * BS + c])

  def foreach_in_state(state, fn):
    state[:] = [[fn(r, c) for c in range(BS)] for r in range(BS)]

  def shift_rows(state, inv=False):
    copy = [e[:] for e in state]
    Aes.foreach_in_state(state, lambda r, c: copy[(r+c if not inv else r-c) % BS][c])

  def key_expansion(self, key): # 192 (4 * 6)
    assert len(key) == self.BLOCK_SIZE//4//2, len(key)
    Nb, Nr, Nk = self.Nb, self.Nr, self.Nk
    ksb = [0] * (Nb * (Nr + 1)) # uint32[n]; n = 
  ...
  def inv_cipher(self, state, ks):
    ...
    for r in range(Nr-1, -1, -1):
      ...
      Aes.add_round_key(state, ks[r*Nb*4:(r+1)*Nb*4])
      if r: Aes.mix_columns(state, True)
